...
def create_io_load():
    """PostgreSQL 컨테이너에 직접적인 I/O 부하 생성"""
    print("PostgreSQL 컨테이너에 I/O 부하 생성 중...")
    ...
    try:
        ...
        # 임시 테이블 생성
        temp_table = f"temp_io_load_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        ...
        # 대량의 데이터 삽입
        # batch_size = 1000  # 한 번에 삽입할 레코드 수
        # total_records = 10000  # 총 삽입할 레코드 수
        batch_size = 100  # 한 번에 삽입할 레코드 수
        total_records = 1000  # 총 삽입할 레코드 수
        record_size = 0.5  # 각 레코드의 크기 (MB)
        
        print(f"부하 생성 설정: 배치 크기={batch_size}, 총 레코드={total_records}, 레코드 크기={record_size}MB")
        
        # I/O 부하를 지속적으로 생성하기 위한 루프
        while True:
            try:
                for i in range(0, total_records, batch_size):
                    ...
                    # 테이블을 비우고 다시 시작 (I/O 부하 생성)
                    cur.execute(f"TRUNCATE TABLE {temp_table}")
                    conn.commit()
                    
                    print(f"I/O 부하 생성 중... {i + batch_size}/{total_records}")
